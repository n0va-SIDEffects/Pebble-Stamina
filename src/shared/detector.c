...
int calib_index(SessionMode mode) {
  switch (mode) {
...
void detector_params(SessionMode mode, const Calib *c, uint8_t sensitivity, DetectorParams *out) {
  detector_params_from_calib(mode, c, out);
  if (sensitivity > SENS_HIGH) sensitivity = SENS_NORMAL;
  out->min_energy = out->min_energy * ENERGY_SCALE[sensitivity] / 10;
  out->min_threshold = out->min_threshold * THRESHOLD_SCALE[sensitivity] / 10;
}
